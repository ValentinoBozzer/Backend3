...
const logout = async (req, res, next) => {
    try {
        ...
        if (!cookie) return res.status(400).send({ status: "error", error: "No hay sesión activa" });
        const userData = jwt.verify(cookie, 'tokenSecretJWT');
        await usersService.update(userData._id, { last_connection: new Date() });
        res.clearCookie('coderCookie').send({ status: "success", message: "Sesión cerrada y last_connection actualizado" });
    } catch (error) {
        next(error);
    }
}
